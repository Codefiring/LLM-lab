...
static int drv\_open\_impl(struct inode *inode, struct file *filp) { return real_open(inode, filp);       // direct call } \end{verbatim} Basic Fact Extraction recovers the direct edge \texttt{drv\_open\_impl() -> real\_open()}, but without resolving the function pointer \texttt{drv\_fops.open}, the edge from \texttt{drv\_open()} to \texttt{drv\_open\_impl()} remains missing. DeepType identifies \texttt{drv\_open\_impl} as the dynamic target of \texttt{drv\_fops.open}, allowing us to add the missing indirect edge and obtain a precise, connected call graph that faithfully reflects the driver’s control flow.
